...
class SACContinuous:
    ...
    def calc_target(self, rewards, next_states, dones):  # 计算目标 Q 值
    ...
    def soft_update(self, net, target_net):
        for param_target, param in zip(target_net.parameters(), net.parameters()): # 由于 parameters() 返回的是迭代器，包含各层的参数，因此需要循环逐层更新
            param_target.data.copy_(param_target.data * (1.0 - self.tau) + param.data * self.tau) # 调用 data 可以改变 tensor ，调用 copy 可以直接更新 tensor
    ...
    def update(self, transition_dict):
        ...
        rewards = (rewards + 8.0) / 8.0 # ？？？不懂意义在哪？可能是为了稳定训练吧 # 结果表明，是否处理 rewards 并不影响最终的训练结果
        # 更新两个Q网络
