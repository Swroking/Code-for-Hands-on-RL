...
class PETS:
    ...
    def mpc(self):
        ...
        while not done:
            ...
            next_obs, reward, done, truncated, _ = self._env.step(action) # 把 动作 传入真实环境
...
plan_horizon = 25 - 15 # 规划未来的步长数
num_episodes = 10 + 5 # 迭代次数
env_name = 'Pendulum-v1' # 状态空间 3 动作空间 1
...
# 即输入 (s, a) 后预测 (r, s') # 这样，就可以在不与真实环境频繁交互的情况下，基于模型进行采样和优化)
# # 具体流程如下
